...
function App(){
  ...
  useEffect(()=>{
    ...
    const handleSubmit = () => {
      ...
      if(editingId){
        setProducts(products.map(product =>product.id === editingId ? {...formData,id:editingId,price:parseFloat(formData.price)||0} : product));
      }
      else{
        const newProduct={
          id:Date.now(),...formData,price:parseFloat(formData.price)||0};
  ...
  return (
    <div className="body">
      ...
      <button className='btn' onClick={()=>setShowForm(true)}>Add New Product<PackagePlusIcon size={20} /></button>
      ...
      {showForm &&(
        <div className="modal" onClick={(e)=>e.target===e.currentTarget && resetForm()}>
          <div className="modal-content">
            ...
            <input type="url" name="image" placeholder="Image URL(optional)" value={formData.image} onChange={handleInputChange} />
            <input type="number" name="price" placeholder="Price" set="0.01" min="0" value={formData.price} onChange={handleInputChange} required/>
            <textarea name="info" placeholder="Product Info" value={formData.info} onChange={handleInputChange}></textarea>
            ...
            <div className="product-card" key={product.id}>
              <div className="card-image-container">
                {product.image&&(
                  ...
                  onError={(e)=>{
                    e.target.style.dsiplay='none';
                    e.target.nextElemnetSibiling.style.display='flex'
                  }}
                  />
                )}
                {product.image&&(
